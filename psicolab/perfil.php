...
$user = $_SESSION['user'];
$query = $db->prepare("SELECT correo_electronico, nombre FROM usuarios WHERE nombre = :user");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['logout'])) {
        ...
        $deleteUserQuery = $db->prepare("DELETE FROM usuarios WHERE nombre = :user");
        $deleteUserQuery->bindParam(':user', $user);
        ...
        $updateNameQuery = $db->prepare("UPDATE usuarios SET nombre = :new_name WHERE nombre = :user");
        $updateNameQuery->bindParam(':new_name', $newName);
        $updateNameQuery->bindParam(':user', $user);
        
        if ($updateNameQuery->execute()) {
            header("Location: login.php");
            exit();
        } else {
...
<section>
  ...
  ?>
    ...
    <div class="container">
        <h1>Bienvenido, <?php echo $userData['nombre']; ?></h1>
        <table class="table">
            <tr>
                <th>Correo Electrónico</th>
                <td><?php echo $userData['correo_electronico']; ?></td>
            </tr>
            <tr>
    ...
    <button type="submit" name="delete_account" class="btn btn-warning">Eliminar Cuenta</button>
</form>


        <form method="post">
            <div class="mb-3">
                <label for="new_name" class="form-label">Nuevo Nombre de Usuario</label>
                <input type="text" class="form-control" id="new_name" name="new_name" required>
            </div>
            <button type="submit" name="update_name" class="btn btn-primary">Modificar Nombre de Usuario</button>
        </form>
    </div>
    <div class="row">
    ...
    function confirmDelete(){
        var confirmDelete = confirm("¿Realmente quieres eliminar tu cuenta?");
