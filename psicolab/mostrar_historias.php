...
$historiasPorPagina = 10;
$paginaActual = isset($_GET['pagina']) ? $_GET['pagina'] : 1;
$inicio = ($paginaActual - 1) * $historiasPorPagina;

$stmt = $db->query("SELECT * FROM HistoriasExito JOIN Usuarios ON HistoriasExito.usuario_id = Usuarios.id ORDER BY HistoriasExito.fecha DESC LIMIT $inicio, $historiasPorPagina");
$historias = $stmt->fetchAll();

$totalHistorias = $db->query("SELECT COUNT(*) FROM HistoriasExito")->fetchColumn();
        ...
        <?php foreach ($historias as $historia) : ?>
            <div class="card mb-3">
                <div class="card-body">
                    ...
                    <p class="card-text"><small class="text-muted">Por: <?php echo $historia['nombre']; ?> - <?php echo $historia['fecha']; ?></small></p>
                </div>
            </div>
        <?php endforeach; ?>
           ...
           <nav aria-label="Paginación">
            <ul class="pagination justify-content-center">
                ...
                for ($i = 1; $i <= $totalPaginas; $i++) {
                    echo "<li class='page-item " . ($i == $paginaActual ? 'active' : '') . "'><a class='page-link' href='exito.php?pagina=$i'>$i</a></li>";
                }
                ?>
            </ul>
        </nav>
